...
class seq_pic2seq_pic():
    ...
    def steps(self, sess, data_dict, noise=None, step_type='train', qa_transpose=False, img_affect_testing=None):
        ...
        if step_type == 'test':
            output_batch_txt_in = np.zeros((self._batch_size, self._sentence_size), dtype=np.int32)
            ...
            for j in range(self._sentence_size):
                ...
                loss,txt_preds = sess.run([self.losses,self.predict_txt],
                ...
                output_batch_txt_in[:, j] = txt_preds[:, j]
            ...
            return loss,txt_preds #, pic_encoding, word_defined_image, acc_img
